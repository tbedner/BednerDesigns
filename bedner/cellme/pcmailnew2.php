...
$lname = $name[1];
  $conn = db_connect();
  $result = $conn->query("SELECT ent_id FROM add_book WHERE fname LIKE '".$fname."' AND lname LIKE '".$lname."' AND username = '".$_SESSION['valid_user']."'");
...
if (empty($to2)) {
 ...
 header("Location: pcemail.php?sent=false");
  ...
  mysqli_close($conn);
 header("Location: pcemail.php?sent=true");
 }
?>